...
export function setRNConfiguration(config) {
  RNCGeolocation.setConfiguration({ ...config,
    authorizationLevel: (config === null || config === void 0 ? void 0 : config.authorizationLevel) === 'auto' ? undefined : config.authorizationLevel,
    locationProvider: (config === null || config === void 0 ? void 0 : config.locationProvider) === 'auto' ? undefined : config.locationProvider
  });
}
/*
...
export function requestAuthorization() {
  let success = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : () => {};
  let error = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : logError;
...
export async function getCurrentPosition(success) {
  let error = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : logError;
  let options = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {};
  invariant(typeof success === 'function', 'Must provide a valid geo_success callback.'); // Permission checks/requests are done on the native side
...
export function watchPosition(success) {
  let error = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : logError;
  let options = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {};
...
export function clearWatch(watchID) {
  ...
  sub[0].remove(); // array element refinements not yet enabled in Flow
